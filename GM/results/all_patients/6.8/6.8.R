...
total <- new_patients %>% group_by(year_group) %>% 
  summarise(rate_1000_pop = sum(current_microvasc)/1000,
            `standard deviation` = sd(current_microvasc))
...
total_m <- new_patients %>% filter(gender==1) %>% group_by(year_group) %>% 
  summarise(rate_1000_pop_Males = sum(current_microvasc)/1000,
            `standard deviation Males` = sd(current_microvasc))
...
total_f <- new_patients %>% filter(gender==2) %>% group_by(year_group) %>% 
  summarise(rate_1000_pop_Females = sum(current_microvasc)/1000,
            `standard deviation Females` = sd(current_microvasc))

all_years <- new_patients %>% 
  summarise(rate_1000_pop = sum(current_microvasc, na.rm=T)/1000,
            `standard deviation` = sd(current_microvasc, na.rm=T))
...
total <- cbind(total[1:4,], total_m[,2:3], total_f[,2:3])

total <- rbind((total),all_years)

write_csv(total, "results/new_patients/6.8/6.8.csv")
