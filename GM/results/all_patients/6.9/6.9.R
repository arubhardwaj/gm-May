...
total <- new_patients %>% group_by(year_group) %>% 
  summarise(rate_1000_pop = sum(current_lvd,na.rm=T)/1000,
            `standard deviation` = sd(current_lvd,na.rm=T)) %>% na.omit()
...
total_m <- new_patients %>% filter(gender==1) %>% group_by(year_group) %>% 
  summarise(rate_1000_pop_Males = sum(current_lvd,na.rm=T)/1000,
            `standard deviation Males` = sd(current_lvd,na.rm=T))
...
all_years <- new_patients %>% 
  summarise(rate_1000_pop = sum(current_lvd,na.rm = T)/1000,
            `standard deviation` = sd(current_lvd,na.rm=T))
...
write_csv(total, "results/new_patients/6.9/6.9.csv")
