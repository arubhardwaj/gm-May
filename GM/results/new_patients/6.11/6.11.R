...
total <- new_patients %>% group_by(year_group) %>% 
  summarise(rate_1000_pop = sum(current_stroke)/1000,
            `standard deviation` = sd(current_stroke)) %>% na.omit()
...
total_m <- new_patients %>% filter(gender==1) %>% group_by(year_group) %>% 
  summarise(rate_1000_pop_Males = sum(current_stroke)/1000,
            `standard deviation Males` = sd(current_stroke))
...
total_f <- new_patients %>% filter(gender==2) %>% group_by(year_group) %>% 
  summarise(rate_1000_pop_Females = sum(current_stroke)/1000,
            `standard deviation Females` = sd(current_stroke))

all_years <- new_patients %>% 
  summarise(rate_1000_pop = sum(current_stroke,na.rm=T)/1000,
            `standard deviation` = sd(current_stroke,na.rm=T))
...
write_csv(total, "results/new_patients/6.11/6.11.csv")
