...
total <- new_patients %>% group_by(year_group) %>% 
  summarise(rate_1000_pop = sum(current_angina)/1000,
            `standard deviation` = sd(current_angina)) %>% na.omit()
...
total_m <- new_patients %>% filter(gender==1) %>% group_by(year_group) %>% 
  summarise(rate_1000_pop_Males = sum(current_angina)/1000,
            `standard deviation Males` = sd(current_angina))
...
total_f <- new_patients %>% filter(gender==2) %>% group_by(year_group) %>% 
  summarise(rate_1000_pop_Females = sum(current_angina)/1000,
            `standard deviation Females` = sd(current_angina))

all_years <- new_patients %>% 
  summarise(rate_1000_pop = sum(current_angina,na.rm=T)/1000,
            `standard deviation` = sd(current_angina,na.rm=T))
...
write_csv(total, "results/new_patients/6.12/6.12.csv")
