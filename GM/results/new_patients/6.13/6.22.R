...
patients_hes <- new_patients %>% mutate(MI_HES = ifelse(is.na(current_mi_HES_event)==TRUE,0,1),
...
any_reason <- data.frame(total[1:4,], total_m[,2:3], total_f[,2:3])
...
LVD_Hospitalisation <- data.frame(total[1:4,], total_m[,2:3], total_f[,2:3])
...
MI_Hospitalisation <- data.frame(total[1:4,], total_m[,2:3], total_f[,2:3])
...
cancer_Hospitalisation <- data.frame(total[1:4,], total_m[,2:3], total_f[,2:3])
...
write_csv(table_6.22, "results/new_patients/6.13/6.22.csv")
