...
total <- new_patients %>% group_by(year_group) %>% 
  summarise(rate_1000_pop = sum(current_cancer)/1000,
            `standard deviation` = sd(current_cancer)) %>% na.omit()
...
total_m <- new_patients %>% filter(gender==1) %>% group_by(year_group) %>% 
  summarise(rate_1000_pop_Males = sum(current_cancer)/1000,
            `standard deviation Males` = sd(current_cancer))
...
total_f <- new_patients %>% filter(gender==2) %>% group_by(year_group) %>% 
  summarise(rate_1000_pop_Females = sum(current_cancer)/1000,
            `standard deviation Females` = sd(current_cancer))

all_years <- new_patients %>% 
  summarise(rate_1000_pop = sum(current_cancer,na.rm = T)/1000,
            `standard deviation` = sd(current_cancer,na.rm = T))
...
write_csv(total, "results/new_patients/6.23/6.23.csv")
