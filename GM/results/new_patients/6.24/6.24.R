...
total <- new_patients %>% group_by(year_group) %>% 
  summarise(rate_1000_pop = sum(dead)/1000,
            `standard deviation` = sd(dead))
...
total_m <- new_patients %>% filter(gender==1) %>% group_by(year_group) %>% 
  summarise(rate_1000_pop_Males = sum(dead)/1000,
            `standard deviation Males` = sd(dead))
...
total_f <- new_patients %>% filter(gender==2) %>% group_by(year_group) %>% 
  summarise(rate_1000_pop_Females = sum(dead)/1000,
            `standard deviation Females` = sd(dead))
...
total <- rbind(total[1:4,],all_years)

write_csv(total, "results/new_patients/6.24/6.24.csv")
